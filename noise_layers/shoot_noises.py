...
def Random_resize(imgs):
    ...
    图像放缩失真
    """
    resize_to = [(64, 64), (224, 224), (256, 256), (384, 384), (512, 512)]
    r = random.randint(0, 4)
    print(resize_to[r])
    imgs_r = F.interpolate(imgs, size=resize_to[r])
    imgs_r = F.interpolate(imgs, size=(128, 128))
    return imgs_r
...
def get_data_loaders(path):
    ...
    valid_loader = torch.utils.data.DataLoader(valid_images, batch_size=2, 
                                                shuffle=False, num_workers=0)

    return valid_loader
def save_images(imgs, folder, name, num=0, resize_to=None):
    """
    ...
    for i in range(num, num+imgs.shape[0]):
        filename = os.path.join(folder, name+'-{}.png'.format(i))
...
if __name__ == '__main__':
    ...
    for imgs, _ in test_data:
        ...
        t2 = time.time()
        print('计算时间：%.3f'%(t2-t1))
        noised_imgs = model(imgs, 400)
        # save_images(noised_imgs-imgs,'./test', 'noise', num, resize_to=(256, 256))
        num += imgs.shape[0]
        epoch += 1
        if epoch > 400:
            break